#-----------------------------------------------------------------------------#
# program : logit.R
# Purpose : build logistic regression classifier.
...
# set data partion - 67% vs 33%set.seed(20200520)
...
logit_pred <- predict.train(logit_model,
                  x_test,
                  type='raw')

# confusion matrix
confusionMatrix(as.factor(logit_pred), as.factor(y_test))

# roc curve
roc_logit <- roc(as.vector(y_test), as.numeric(logit_pred))
...
new_data <- data.frame(
  ...
  ,normal_nucleoli = 8
  ,mitoses         = 10)
pred_logit_new <- predict.train(logit_model,
